    ...
    public static CallerInfo getCallerInfo(Context context, Uri contactRef, Cursor cursor) {
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                boolean z = columnIndex != -1 && cursor.getInt(columnIndex) == 1;
    ...
    private static String getGeoDescription(Context context, String number) {
        ...
        if (!TextUtils.isEmpty(number)) {
            ...
            try {
                ...
                Rlog.w(TAG, "getGeoDescription: NumberParseException for incoming number '<MASKED>'");
            }
            if (pn != null) {
                str = geocoder.getDescriptionForNumber(pn, locale);
                if (VDBG) {
                    Rlog.v(TAG, "- got description: '" + str + "'");
                }
            }
        }
        return str;
