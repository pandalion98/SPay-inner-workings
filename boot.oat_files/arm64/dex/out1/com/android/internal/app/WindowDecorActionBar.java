...
public class WindowDecorActionBar extends ActionBar implements ActionBarVisibilityCallback {
    ...
    private static final boolean mIsMRProject;
    ...
    static {
        ...
        if (SystemProperties.get("ro.build.scafe").equals("americano") || SystemProperties.get("ro.build.scafe").equals("mocha")) {
            z2 = true;
        }
        mIsMRProject = z2;
    }
    ...
    private void setHasEmbeddedTabs(boolean hasEmbeddedTabs) {
        ...
        if (this.mActivity != null) {
            if (this.mActivity.checkNoEmbeddedTabs()) {
    ...
    public void doShow(boolean fromSystem) {
        ...
        if (this.mCurWindowVisibility == 0 && (this.mShowHideAnimationEnabled || fromSystem)) {
            ...
            float startingY = (float) ((-this.mContainerView.getContainerHeight()) - 1);
            if (fromSystem) {
    ...
    void animateToMode(boolean toActionMode) {
        ...
        if (!mIsMRProject || !(this.mDecorToolbar instanceof ActionBarView) || this.mTabScrollView == null || this.mDecorToolbar.hasEmbeddedTabs()) {
            Animator fadeOut;
            Animator fadeIn;
            if (toActionMode) {
        ...
        int i3;
        float f2;
        ActionBarView actionBarView = (ActionBarView) this.mDecorToolbar;
        if (toActionMode) {
            i2 = 8;
        } else {
            i2 = 0;
        }
        actionBarView.setVisibility(i2);
        ...
        actionBarContextView.setVisibility(i3);
        actionBarView = (ActionBarView) this.mDecorToolbar;
        if (toActionMode) {
            f2 = 0.0f;
        } else {
            f2 = 1.0f;
        }
        actionBarView.setAlpha(f2);
        ActionBarContextView actionBarContextView2 = this.mContextView;
        if (!toActionMode) {
            f = 0.0f;
        }
        actionBarContextView2.setAlpha(f);
        if ((this.mDecorToolbar.getDisplayOptions() & 8) == 0) {
            showTitle = false;
        }
        if (!showTitle) {
            ScrollingTabContainerView scrollingTabContainerView = this.mTabScrollView;
    ...
    public void twPutTabsOnTop(boolean isTabOnTop) {
        if (mIsMRProject) {
            this.mContainerView.twPutTabsOnTop(isTabOnTop);
        }
    }
}
