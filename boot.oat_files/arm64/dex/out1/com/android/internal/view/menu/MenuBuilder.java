...
public class MenuBuilder implements Menu {
    ...
    private boolean isSupportOverlapSubMenu() {
        return GeneralUtil.isTablet() && this.mIsDeviceDefaultLight && !ViewConfiguration.get(this.mContext).hasPermanentMenuKey();
    }
    ...
    private boolean dispatchSubMenuSelected(SubMenuBuilder subMenu, MenuPresenter preferredPresenter) {
        ...
        if (preferredPresenter != null) {
            if (isSupportOverlapSubMenu()) {
                result = ((ActionMenuPresenter) preferredPresenter).twOnSubMenuSelected(subMenu, this.mSubAnchor);
            } else {
        ...
        while (i$.hasNext()) {
            ...
            if (presenter == null) {
                ...
                continue;
            } else if (!isSupportOverlapSubMenu()) {
                result = presenter.onSubMenuSelected(subMenu);
            } else if (presenter instanceof ActionMenuPresenter) {
                return ((ActionMenuPresenter) presenter).twOnSubMenuSelected(subMenu, this.mSubAnchor);
            } else {
                return ((MenuPopupHelper) presenter).twOnSubMenuSelected(subMenu, this.mSubAnchor);
            }
        }
        return result;
    }
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(R.bool.config_showMenuShortcutsWhenKeyboardPresent))) {
            z = false;
        }
    ...
    public boolean twPerformItemAction(View view, MenuItem item, int flags) {
        this.mSubAnchor = view;
        return performItemAction(item, 0);
    }
    ...
    public boolean performItemAction(MenuItem item, MenuPresenter preferredPresenter, int flags) {
        ...
        if (itemImpl.hasCollapsibleActionView()) {
            ...
            if (isSupportOverlapSubMenu()) {
                ...
                subMenu = (SubMenuBuilder) itemImpl.getSubMenu();
                if (providerHasSubMenu) {
                    provider.onPrepareSubMenu(subMenu);
                }
                return invoked | dispatchSubMenuSelected(subMenu, preferredPresenter);
            }
            close(false);
            if (!itemImpl.hasSubMenu()) {
                itemImpl.setSubMenu(new SubMenuBuilder(getContext(), this, itemImpl));
    ...
    public final void close(boolean allMenusAreClosing) {
        if (!this.mIsClosing) {
            ...
            WeakReference<MenuPresenter> ref;
            MenuPresenter presenter;
            if (allMenusAreClosing && isSupportOverlapSubMenu() && this.mPresenters.size() > 0) {
                ref = (WeakReference) this.mPresenters.get(this.mPresenters.size() - 1);
                presenter = (MenuPresenter) ref.get();
                if (presenter == null) {
                    this.mPresenters.remove(ref);
                } else {
