...
public class ViewPager extends ViewGroup {
            ...
            while (curIndex < this.mItems.size()) {
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    for (i = 0; i < this.mItems.size(); i++) {
                        Log.i(TAG, "#" + i + ": page " + ((ItemInfo) this.mItems.get(i)).position);
                    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = 0; i < size; i++) {
            child = getChildAt(i);
            if (child.getVisibility() != 8) {
                Log.v(TAG, "Measuring #" + i + " " + child + ": " + this.mChildWidthMeasureSpec);
                lp = (LayoutParams) child.getLayoutParams();
                if (lp == null || !lp.isDecor) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case 0:
                ...
                Log.v(TAG, "Down at " + this.mLastMotionX + FingerprintManager.FINGER_PERMISSION_DELIMITER + this.mLastMotionY + " mIsBeingDragged=" + this.mIsBeingDragged + "mIsUnableToDrag=" + this.mIsUnableToDrag);
                break;
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    Log.v(TAG, "Moved x to " + x2 + FingerprintManager.FINGER_PERMISSION_DELIMITER + y + " diff=" + xDiff + FingerprintManager.FINGER_PERMISSION_DELIMITER + yDiff);
    ...
    private boolean performDrag(float x) {
        ...
        if (!startAbsolute) {
            ...
            startBound = (startItem.offset * ((float) width)) + ((float) width);
        } else {
        ...
        if (!endAbsolute) {
            ...
            endBound = (endItem.offset * ((float) width)) + ((float) width);
        } else {
