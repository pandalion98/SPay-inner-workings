...
public class PointerLocationView extends View implements InputDeviceListener, PointerEventListener {
    ...
    private final ArrayList<PointerState> mPointers = new ArrayList<PointerState>() {
        public PointerState get(int index) {
            try {
                return (PointerState) super.get(index);
            } catch (Exception ex) {
                Log.e(PointerLocationView.TAG, "Failed to access mPointers.get(" + index + ") " + ex);
                ex.printStackTrace();
                return size() > 0 ? (PointerState) super.get(0) : new PointerState();
            }
        }
    };
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mActivePointerId >= 0) {
            ...
            canvas.drawRect((float) (itemW * 6), 0.0f, (float) w, (float) bottom, this.mTextBackgroundPaint);
    ...
    private void logCoords(String type, int action, int index, PointerCoords coords, int id, MotionEvent event) {
        ...
        switch (action & 255) {
            case 0:
                prefix = DirectionLockTouchListener.DOWN;
                break;
            case 1:
                prefix = DirectionLockTouchListener.UP;
                break;
            case 2:
                prefix = "MOVE";
                break;
