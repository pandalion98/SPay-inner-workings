...
public class RenderNode {
    ...
    private static native void nAddImageFilterClipRect(long j, int i, int i2, int i3, int i4, float f);

    private static native void nClearImageFilterClipRects(long j);

    private static native void nClearImageFilters(long j);
    ...
    private static native void nSetImageFilter(long j, long j2, int i);
    ...
    public DisplayListCanvas start(int width, int height) {
        ...
        if (!(this.mOwningView == null || this.mOwningView.mAttachInfo == null)) {
            canvas.setHighContrastText(this.mOwningView.mAttachInfo.mHighContrastText);
        }
    ...
    void setImageFilter(ImageFilter filter) {
        nClearImageFilters(this.mNativeRenderNode);
        processImageFilter(filter);
    }

    private void processImageFilter(ImageFilter filter) {
        if (filter == null) {
            return;
        }
        if (filter instanceof ImageFilterSet) {
            ImageFilterSet filterSet = (ImageFilterSet) filter;
            for (int i = 0; i < filterSet.getFilterCount(); i++) {
                processImageFilter(filterSet.getFilterAt(i));
            }
            return;
        }
        nSetImageFilter(this.mNativeRenderNode, filter.mNativeImageFilter, 0);
    }

    void clearImageFilterClipRects() {
        nClearImageFilterClipRects(this.mNativeRenderNode);
    }

    void addImageFilterClipRect(int l, int t, int w, int h, float radius) {
        nAddImageFilterClipRect(this.mNativeRenderNode, l, t, w, h, radius);
    }
