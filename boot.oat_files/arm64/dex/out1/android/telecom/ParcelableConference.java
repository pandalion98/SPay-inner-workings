...
public final class ParcelableConference implements Parcelable {
    public static final Creator<ParcelableConference> CREATOR = new Creator<ParcelableConference>() {
        public ParcelableConference createFromParcel(Parcel source) {
            ...
            return new ParcelableConference(phoneAccount, state, capabilities, connectionIds, Stub.asInterface(source.readStrongBinder()), source.readInt(), source.readLong(), (StatusHints) source.readParcelable(classLoader), source.readBundle(classLoader));
        }
    ...
    public ParcelableConference(PhoneAccountHandle phoneAccount, int state, int connectionCapabilities, List<String> connectionIds, IVideoProvider videoProvider, int videoState, long connectTimeMillis, StatusHints statusHints, Bundle extras) {
