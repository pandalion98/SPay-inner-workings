...
public abstract class WallpaperService extends Service {
    ...
    private boolean SUPPORT_MOBILE_KEYBOARD = FloatingFeature.getInstance().getEnableStatus("SEC_FLOATING_FEATURE_COMMON_SUPPORT_NFC_HW_KEYBOARD");
    ...
    public class Engine {
        ...
        final BaseSurfaceHolder mSurfaceHolder = new BaseSurfaceHolder() {
            ...
            public void onRelayoutContainer() {
                Log.d(WallpaperService.TAG, "BaseSurfaceHolder:onRelayoutContainer");
                Engine.this.mCaller.sendMessage(Engine.this.mCaller.obtainMessage(10000));
            }

            public void onUpdateSurface() {
                Log.d(WallpaperService.TAG, "BaseSurfaceHolder:onUpdateSurface");
                Engine.this.mCaller.sendMessage(Engine.this.mCaller.obtainMessage(10000));
            }
            ...
            public void setFixedSize(int width, int height) {
                if (Engine.this.mFixedSizeAllowed) {
                    Log.d(WallpaperService.TAG, "BaseSurfaceHolder:setFixedSize[width]" + width + " [height]" + height);
        ...
        final BaseIWindow mWindow = new BaseIWindow() {
            public void resized(Rect frame, Rect overscanInsets, Rect contentInsets, Rect visibleInsets, Rect stableInsets, Rect outsets, boolean reportDraw, Configuration newConfig, Rect cocktailBarFrame) {
                if (newConfig != null) {
                    Log.d(WallpaperService.TAG, "resized:[frame]" + frame + " [newConfig]" + newConfig.orientation);
                } else {
                    Log.d(WallpaperService.TAG, "resized:[frame]" + frame + " [newConfig] is null");
                }
        ...
        public int getDisplayId() {
            Log.d(WallpaperService.TAG, "getDisplayId:" + this.mIWallpaperEngine.mDisplayId);
        ...
        void updateSurface(boolean forceRelayout, boolean forceReport, boolean redrawNeeded) {
            Log.d(WallpaperService.TAG, "updateSurface:[forceRelayout]" + forceRelayout + "[redrawNeeded]" + redrawNeeded);
            if (this.mDestroyed) {
            ...
            if (forceRelayout || creating || surfaceCreating || formatChanged || z || typeChanged || flagsChanged || redrawNeeded || !this.mIWallpaperEngine.mShownReported) {
                Log.v(WallpaperService.TAG, "Changes: creating=" + creating + " format=" + formatChanged + " size=" + z);
                Log.v(WallpaperService.TAG, "currentWidth:" + this.mWidth + " currentHeight:" + this.mHeight + " requestedWidth:" + myWidth + " requestedWidth:" + myHeight);
                try {
                    Rect rect;
                    this.mWidth = myWidth;
                    ...
                    if (!this.mCreated) {
                        ...
                        if (this.mSession.addToDisplay(this.mWindow, this.mWindow.mSeq, this.mLayout, 0, this.mDisplay.getDisplayId(), this.mContentInsets, this.mStableInsets, this.mOutsets, this.mInputChannel) < 0) {
                    ...
                    int relayoutResult = this.mSession.relayout(this.mWindow, this.mWindow.mSeq, this.mLayout, this.mWidth, this.mHeight, 0, 0, this.mWinFrame, this.mOverscanInsets, this.mContentInsets, this.mVisibleInsets, this.mStableInsets, this.mOutsets, this.mConfiguration, this.mSurfaceHolder.mSurface, new PointF(), new Rect());
                    Log.v(WallpaperService.TAG, "relayout result: " + this.mSurfaceHolder.mSurface + ", frame=" + this.mWinFrame + ",relayoutResult:" + relayoutResult + ", mConfiguration" + this.mConfiguration);
                    ...
                    Log.v(WallpaperService.TAG, "Wallpaper size has changed: (" + this.mCurWidth + ", " + this.mCurHeight);
                    insetsChanged = (((insetsChanged | (!this.mDispatchedOverscanInsets.equals(this.mOverscanInsets) ? 1 : 0)) | (!this.mDispatchedContentInsets.equals(this.mContentInsets) ? 1 : 0)) | (!this.mDispatchedStableInsets.equals(this.mStableInsets) ? 1 : 0)) | (!this.mDispatchedOutsets.equals(this.mOutsets) ? 1 : 0);
                    ...
                    if (this.mSurfaceHolder.mSurface.isValid()) {
                        ...
                        if (forceReport || creating || surfaceCreating || formatChanged || sizeChanged) {
                            Log.v(WallpaperService.TAG, "onSurfaceChanged(" + this.mSurfaceHolder + ", " + this.mFormat + ", " + this.mCurWidth + ", " + this.mCurHeight + "): " + this);
                            didSurface = true;
                        ...
                        if (insetsChanged) {
                            ...
                            onApplyWindowInsets(new WindowInsets(this.mFinalSystemInsets, null, this.mFinalStableInsets, WallpaperService.this.getResources().getConfiguration().isScreenRound()));
                        }
                        if (redrawNeeded) {
        ...
        void attach(IWallpaperEngineWrapper wrapper) {
            if (!this.mDestroyed) {
                ...
                this.mDisplay = this.mDisplayManager.getDisplay(wrapper.mDisplayId);
    ...
    class IWallpaperEngineWrapper extends Stub implements HandlerCaller.Callback {
        ...
        public void executeMessage(Message message) {
            switch (message.what) {
                ...
                case 10030:
                    boolean reportDraw;
                    Log.d(WallpaperService.TAG, "MSG_WINDOW_RESIZED");
                    if (message.arg1 != 0) {
                        reportDraw = true;
                    } else {
                        reportDraw = false;
            ...
            Log.d(WallpaperService.TAG, "MSG_UPDATE_SURFACE");
    ...
    class IWallpaperServiceWrapper extends IWallpaperService.Stub {
        ...
        public void attach(IWallpaperConnection conn, IBinder windowToken, int windowType, boolean isPreview, int reqWidth, int reqHeight, Rect padding) {
            IWallpaperEngineWrapper iWallpaperEngineWrapper = new IWallpaperEngineWrapper(this.mTarget, conn, windowToken, windowType, isPreview, reqWidth, reqHeight, padding, 0);
        }

        public void attachWithDisplayId(IWallpaperConnection conn, IBinder windowToken, int windowType, boolean isPreview, int reqWidth, int reqHeight, Rect padding, int displayId) {
            IWallpaperEngineWrapper iWallpaperEngineWrapper = new IWallpaperEngineWrapper(this.mTarget, conn, windowToken, windowType, isPreview, reqWidth, reqHeight, padding, displayId);
        }
    }
