...
public class DreamService extends Service implements Callback {
    ...
    private int mStartScreenState = -1;
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (!this.mInteractive) {
            ...
            wakeUp();
            return true;
        } else if (event.getKeyCode() != 4 && event.getKeyCode() != 3) {
            return this.mWindow.superDispatchKeyEvent(event);
        } else {
            if (this.mDebug) {
                Slog.v(this.TAG, "Waking up back or home key");
            }
            wakeUp();
            return true;
        }
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent event) {
        ...
        if (event.getAction() == 7 || event.getAction() == 9 || event.getAction() == 10) {
            return false;
        }
        wakeUp();
        return true;
