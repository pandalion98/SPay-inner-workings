...
public class TextKeyListener extends BaseKeyListener implements SpanWatcher {
    ...
    private void updatePrefs(ContentResolver resolver) {
        ...
        int i;
        int i2 = 0;
        if (Settings$System.getInt(resolver, Settings$System.TEXT_AUTO_CAPS, 1) > 0) {
            cap = true;
        } else {
            cap = false;
        }
        if (Settings$System.getInt(resolver, Settings$System.TEXT_AUTO_REPLACE, 1) > 0) {
            text = true;
        } else {
            text = false;
        }
        if (Settings$System.getInt(resolver, Settings$System.TEXT_AUTO_PUNCTUATE, 1) > 0) {
            period = true;
        } else {
            period = false;
        }
        if (Settings$System.getInt(resolver, Settings$System.TEXT_SHOW_PASSWORD, 1) > 0) {
            pw = true;
        } else {
            pw = false;
        }
