...
public abstract class MetaKeyKeyListener {
    ...
    public static long handleKeyDown(long state, int keyCode, KeyEvent event) {
        if (keyCode == 59 || keyCode == 60) {
            return press(state, 1, META_SHIFT_MASK, 256, META_CAP_PRESSED, META_CAP_RELEASED, META_CAP_USED);
        }
        if (keyCode == 57 || keyCode == 58 || keyCode == 78) {
            return press(state, 2, META_ALT_MASK, 512, META_ALT_PRESSED, META_ALT_RELEASED, META_ALT_USED);
        }
        if (keyCode == 63) {
            return press(state, 4, META_SYM_MASK, 1024, META_SYM_PRESSED, META_SYM_RELEASED, META_SYM_USED);
        }
        return state;
    }
