...
class MeasuredText {
    ...
    private static final Pattern phoneNum1 = Pattern.compile("[+][0-9]+");
    private static final Pattern phoneNum2 = Pattern.compile("[0-9]+([-/*+=. ]+([0-9]+))+");
    ...
    void setPara(CharSequence text, int start, int end, TextDirectionHeuristic textDir, Builder builder) {
        ...
        if (this.mDir == -1) {
            int startIndex;
            String newStr = new String(this.mChars, 0, len);
            Matcher m = phoneNum1.matcher(newStr);
            while (m.find()) {
                startIndex = m.start();
                if (startIndex >= 0 && startIndex < len) {
                    this.mLevels[startIndex] = this.mLevels[startIndex + 1];
                }
            }
            m = phoneNum2.matcher(newStr);
            while (m.find()) {
                startIndex = m.start();
                int endIndex = m.end() - 1;
                if (startIndex >= 0 && startIndex < len && endIndex >= 0 && endIndex < len) {
                    for (i = startIndex; i <= endIndex; i++) {
                        this.mLevels[i] = this.mLevels[startIndex];
                    }
                }
            }
        }
    ...
    float addStyleRun(TextPaint paint, int len, FontMetricsInt fm) {
        ...
        if (this.mEasy) {
            ...
            if (len < 1 || !TextUtils.isRegionalCharHandling(this.mChars[len - 1])) {
                return width;
            }
            cPos = len - 1;
            spaceWid = paint.measureText(" ");
            if (widths != null) {
                while (widths[cPos] == 0.0f) {
                    cPos--;
                }
                widths[cPos] = widths[cPos] + spaceWid;
            }
            return width + spaceWid;
        }
