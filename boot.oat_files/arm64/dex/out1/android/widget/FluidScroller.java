...
import com.samsung.android.visualeffect.scroll.FastScrollEffect;
...
class FluidScroller {
    ...
    private static final int DURATION_FADE_IN = 333;
    private static final int DURATION_FADE_OUT = 100;
    ...
    private static final String TAG = "FluidScroller";
    ...
    private final int EFFECT_STATE_CLOSE = 0;
    private final int EFFECT_STATE_OPEN = 1;
    ...
    private final Runnable mDeferHide = new Runnable() {
        public void run() {
            FluidScroller.this.setState(0);
        }
    };
    private int mEffectState = 0;
    private boolean mEnabled;
    private FastScrollEffect mFastScrollEffect;
    private String mFastScrollEffectText;
    ...
    private final AnimatorListener mSwitchPrimaryListener = new AnimatorListenerAdapter() {
        public void onAnimationEnd(Animator animation) {
            FluidScroller.this.mShowingPrimary = !FluidScroller.this.mShowingPrimary;
        }
    };
    ...
    public FluidScroller(AbsListView listView, int styleResId) {
        boolean z = true;
        this.mList = listView;
    ...
    private void updateAppearance() {
        ...
        this.mContext.getTheme().resolveAttribute(R.attr.colorPrimary, outValue, true);
        this.mColorPrimary = this.mContext.getResources().getColor(outValue.resourceId);
        this.mFastScrollEffect.setColor(getColorWithAlpha(this.mColorPrimary, MultiWindowFacade.SPLIT_MAX_WEIGHT));
        this.mFastScrollEffect.setListTextColor(this.mContext.getResources().getColor(R.color.tw_visual_effect_text_color));
        this.mFastScrollEffect.setListTextSize(this.mContext.getResources().getDimension(R.dimen.tw_fluid_scroller_text_size));
        this.mOrientation = this.mContext.getResources().getConfiguration().orientation;
        refreshDrawablePressedState();
    }

    public void setStyle(int resId) {
        TypedArray ta = this.mContext.obtainStyledAttributes(null, R.styleable.FastScroll, R.attr.fastScrollStyle, resId);
    ...
    public void remove() {
        ...
        this.mOverlay.remove(this.mFastScrollEffect);
    }
    ...
    private void updateLongList(int childCount, int itemCount) {
        boolean longList = true;
        if (childCount <= 0 || !(this.mList.canScrollList(1) || this.mList.canScrollList(-1))) {
            longList = false;
        }
        if (this.mLongList != longList) {
            this.mLongList = longList;
            if (!(this.mLongList || this.mAlwaysShow)) {
                this.mFastScrollEffect.clearEffect();
                Log.d(TAG, "updateLongList() clearEffect !!!");
            }
            onStateDependencyChanged(false);
        }
    }
    ...
    public void updateLayout() {
        if (!this.mUpdatingLayout) {
            this.mFastScrollEffect.clearEffect();
            Log.d(TAG, "updateLayout() clearEffect !!!");
            this.mFastScrollEffect.setLayout((int) this.mList.getX(), (int) this.mList.getY(), this.mList.getWidth(), this.mList.getHeight());
            Log.d(TAG, "updateLayout() l=" + this.mList.getX() + ", t=" + this.mList.getY() + ", w=" + this.mList.getWidth() + ", h=" + this.mList.getHeight());
    ...
    private void transitionToHidden() {
        ...
        Animator fadeOut = groupAnimatorOfFloat(View.ALPHA, 0.0f, this.mThumbImage, this.mTrackImage).setDuration(100);
        if (this.mDecorAnimation != null) {
            this.mDecorAnimation.cancel();
        } else {
            fadeOut = groupAnimatorOfFloat(View.ALPHA, 0.0f, this.mThumbImage, this.mTrackImage).setDuration(0);
        }
        float offset;
        if (this.mLayoutFromRight) {
            offset = (float) this.mThumbImage.getWidth();
        } else {
            offset = (float) (-this.mThumbImage.getWidth());
        }
    ...
    private void transitionToVisible() {
        ...
        Animator fadeIn = groupAnimatorOfFloat(View.ALPHA, 1.0f, this.mThumbImage, this.mTrackImage).setDuration(333);
        this.mDecorAnimation = new AnimatorSet();
    ...
    private void scrollTo(float position) {
        ...
        if (this.mCurrentSection != sectionIndex) {
            ...
            transitionPreviewLayout(sectionIndex);
    ...
    private boolean transitionPreviewLayout(int sectionIndex) {
        ...
        this.mFastScrollEffectText = text;
        Log.d(TAG, "transitionPreviewLayout() sectionIndex = " + sectionIndex + ", mFastScrollEffectText = " + this.mFastScrollEffectText);
        return !TextUtils.isEmpty(text);
    }

    private void setThumbPos(float position) {
        this.mThumbImage.setTranslationY(((this.mThumbRange * position) + this.mThumbOffset) - (((float) this.mThumbImage.getHeight()) / 2.0f));
    }
        ...
        switch (ev.getActionMasked()) {
            ...
            case 2:
                if (!isPointInside(ev.getX(), ev.getY())) {
                    ...
                    this.mFastScrollEffect.open(this.mInitialTouchY, this.mFastScrollEffectText);
        ...
        switch (me.getActionMasked()) {
            case 0:
                if (isPointInside(me.getX(), me.getY()) && !this.mList.isInScrollingContainer()) {
                    transitionToHidden();
                    beginDrag();
                    this.mFastScrollEffect.open(this.mScrollY, this.mFastScrollEffectText);
                    this.mEffectState = 1;
