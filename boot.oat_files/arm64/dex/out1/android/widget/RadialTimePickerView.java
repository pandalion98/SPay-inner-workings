...
public class RadialTimePickerView extends View {
    ...
    private static final int[] SNAP_PREFER_30S_MAP = new int[ImsReasonInfo.CODE_SIP_USER_REJECTED];
    ...
    private class RadialPickerTouchHelper extends ExploreByTouchHelper {
        ...
        protected int getVirtualViewAt(float x, float y) {
            ...
            int snapDegrees = RadialTimePickerView.snapOnly30s(degrees, 0) % DisplayMetrics.DENSITY_360;
            if (RadialTimePickerView.this.mShowHours) {
    ...
    private static void preparePrefer30sMap() {
        ...
        int count = 1;
        int expectedCount = 8;
        for (int degrees = 0; degrees < ImsReasonInfo.CODE_SIP_USER_REJECTED; degrees++) {
            SNAP_PREFER_30S_MAP[degrees] = snappedOutputDegrees;
            if (count == expectedCount) {
                snappedOutputDegrees += 6;
                if (snappedOutputDegrees == DisplayMetrics.DENSITY_360) {
                    expectedCount = 7;
    ...
    public RadialTimePickerView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        int selectorActivatedColor = a.getColorStateList(5).getColorForState(StateSet.get(40), 0);
    ...
    private static ObjectAnimator getFadeInAnimator(IntHolder target, int startAlpha, int endAlpha, InvalidateUpdateListener updateListener) {
        ...
        Keyframe kf1 = Keyframe.ofInt(MultiWindowFacade.SPLIT_MIN_WEIGHT, startAlpha);
    ...
    private int getDegreesFromXY(float x, float y, boolean constrainOutside) {
        ...
        if (degrees < 0) {
            return degrees + DisplayMetrics.DENSITY_360;
        }
        return degrees;
    }
    ...
    private boolean handleTouchInput(float x, float y, boolean forceSelection, boolean autoAdvance) {
        ...
        int snapDegrees;
        if (this.mShowHours) {
            snapDegrees = snapOnly30s(degrees, 0) % DisplayMetrics.DENSITY_360;
            ...
            snapDegrees = snapPrefer30s(degrees) % DisplayMetrics.DENSITY_360;
