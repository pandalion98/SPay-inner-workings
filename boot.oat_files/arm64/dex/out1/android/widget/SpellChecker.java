        ...
        if (this.mSpellCheckerSession != null) {
            ...
            for (int i = 0; i < this.mLength; i++) {
                ...
                if (this.mIds[i] >= 0 && !spellCheckSpan.isSpellCheckInProgress()) {
                    ...
                    boolean apostrophe = selectionStart == end + 1 && editable.charAt(end) == DateFormat.QUOTE;
                    boolean isEditing = this.mIsSentenceSpellCheckSupported ? !apostrophe && (selectionEnd <= start || selectionStart > end) : !apostrophe && (selectionEnd < start || selectionStart > end);
