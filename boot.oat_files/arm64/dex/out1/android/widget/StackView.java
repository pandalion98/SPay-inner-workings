...
public class StackView extends AdapterViewAnimator {
    ...
    private void beginGestureIfNeeded(float deltaY) {
        ...
        if (((int) Math.abs(deltaY)) > this.mTouchSlop && this.mSwipeGestureType == 0) {
            ...
            if (this.mAdapter != null) {
                ...
                } else if (swipeGestureType == 2) {
                    activeIndex = 0;
                } else {
                    activeIndex = 1;
                }
                boolean endOfStack;
                if (this.mLoopViews && adapterCount == 1 && ((this.mStackMode == 0 && swipeGestureType == 1) || (this.mStackMode == 1 && swipeGestureType == 2))) {
                    endOfStack = true;
                } else {
                    endOfStack = false;
                }
                boolean beginningOfStack;
                if (this.mLoopViews && adapterCount == 1 && ((this.mStackMode == 1 && swipeGestureType == 1) || (this.mStackMode == 0 && swipeGestureType == 2))) {
    ...
    private void handlePointerUp(MotionEvent ev) {
        ...
        if (deltaY > this.mSwipeThreshold && this.mSwipeGestureType == 2 && this.mStackSlider.mMode == 0) {
            ...
            finalYProgress = this.mStackMode == 1 ? 1.0f : 0.0f;
            ...
            finalYProgress = this.mStackMode == 1 ? 0.0f : 1.0f;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightSpecMode == 0) {
            if (haveChildRefSize) {
                heightSpecSize = (Math.round(((float) this.mReferenceChildHeight) * (1.0f + 1.1111112f)) + this.mPaddingTop) + this.mPaddingBottom;
            } else {
                heightSpecSize = 0;
            }
        } else if (heightSpecMode == Integer.MIN_VALUE) {
            if (haveChildRefSize) {
                int height = (Math.round(((float) this.mReferenceChildHeight) * (1.0f + 1.1111112f)) + this.mPaddingTop) + this.mPaddingBottom;
        ...
        if (widthSpecMode == 0) {
            if (haveChildRefSize) {
                widthSpecSize = (Math.round(((float) this.mReferenceChildWidth) * (1.0f + 1.1111112f)) + this.mPaddingLeft) + this.mPaddingRight;
            } else {
                widthSpecSize = 0;
            }
        } else if (heightSpecMode == Integer.MIN_VALUE) {
