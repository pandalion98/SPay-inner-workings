...
public abstract class AndroidKeyStoreKeyGeneratorSpi extends KeyGeneratorSpi {
                    ...
                    if (this.mKeySizeBits <= 0) {
                        ...
                        try {
                            ...
                            if ((spec.getPurposes() & 1) != 0 && spec.isRandomizedEncryptionRequired()) {
                                int[] arr$ = this.mKeymasterBlockModes;
                                int len$ = arr$.length;
                                int i$ = 0;
                                while (i$ < len$) {
    ...
    protected SecretKey engineGenerateKey() {
        ...
        try {
            Credentials.deleteAllTypesForAlias(this.mKeyStore, spec.getKeystoreAlias());
            int errorCode = this.mKeyStore.generateKey(keyAliasInKeystore, args, additionalEntropy, 0, resultingKeyCharacteristics);
            if (errorCode != 1) {
                throw new ProviderException("Keystore operation failed", KeyStore.getKeyStoreException(errorCode));
            }
            SecretKey result = new AndroidKeyStoreSecretKey(keyAliasInKeystore, KeyAlgorithm.fromKeymasterSecretKeyAlgorithm(this.mKeymasterAlgorithm, this.mKeymasterDigest));
            if (!true) {
                Credentials.deleteAllTypesForAlias(this.mKeyStore, spec.getKeystoreAlias());
            }
            return result;
