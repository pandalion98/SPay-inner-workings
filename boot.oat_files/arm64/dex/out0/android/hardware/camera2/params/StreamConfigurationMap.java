...
public final class StreamConfigurationMap {
    ...
    private static final int HAL_DATASPACE_JFIF = 257;
    ...
    private int checkArgumentFormatSupported(int format, boolean output) {
        ...
        if (!output) {
            if (this.mInputFormats.indexOfKey(internalFormat) >= 0) {
            }
        ...
        } else if (internalDataspace == 4096) {
        }
        return format;
    }
    ...
    private Size[] getInternalFormatSizes(int format, int dataspace, boolean output, boolean highRes) {
        ...
        StreamConfiguration[] arr$ = dataspace == 4096 ? this.mDepthConfigurations : this.mConfigurations;
        int len$ = arr$.length;
        int i$ = 0;
        int sizeIndex = 0;
        while (i$ < len$) {
            ...
            if (fmt == format && config.isOutput() == output) {
                if (output && this.mListHighResolution) {
                    boolean z;
                    long duration = 0;
                    for (StreamConfigurationDuration d : this.mMinFrameDurations) {
    ...
    private void appendValidOutputFormatsForInputString(StringBuilder sb) {
        ...
        for (int inputFormat : getInputFormats()) {
            sb.append(String.format("[in:%s(%d), out:", new Object[]{formatToString(inputFormat), Integer.valueOf(arr$[i$])}));
