...
public abstract class BlobBackupHelper implements BackupHelper {
    ...
    private ArrayMap<String, Long> readOldState(ParcelFileDescriptor oldStateFd) {
        ...
        DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(oldStateFd.getFileDescriptor())));
        try {
    ...
    private void writeBackupState(ArrayMap<String, Long> state, ParcelFileDescriptor stateFile) {
        try {
            ...
            out.writeInt(N);
            for (int i = 0; i < N; i++) {
                out.writeUTF((String) state.keyAt(i));
                out.writeLong(((Long) state.valueAt(i)).longValue());
            }
        } catch (IOException e) {
