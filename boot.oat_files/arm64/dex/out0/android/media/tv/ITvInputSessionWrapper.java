...
public class ITvInputSessionWrapper extends Stub implements Callback {
        ...
        if (this.mTvInputSessionImpl != null) {
            ...
            if (duration > 50) {
                ...
                if (msg.what == 6 && duration > 2000) {
                    throw new RuntimeException("Too much time to handle tune request. (" + duration + "ms > " + 2000 + "ms) " + "Consider handling the tune request in a separate thread.");
                } else if (duration > 5000) {
                    throw new RuntimeException("Too much time to handle a request. (type=" + msg.what + ", " + duration + "ms > " + 5000 + "ms).");
                }
            }
        }
    }
    ...
    public void timeShiftPause() {
        this.mCaller.executeOrSendMessage(this.mCaller.obtainMessage(14));
    }
    ...
    public void timeShiftSeekTo(long timeMs) {
        this.mCaller.executeOrSendMessage(this.mCaller.obtainMessageO(16, Long.valueOf(timeMs)));
    }

    public void timeShiftSetPlaybackParams(PlaybackParams params) {
        this.mCaller.executeOrSendMessage(this.mCaller.obtainMessageO(17, params));
    }

    public void timeShiftEnablePositionTracking(boolean enable) {
        this.mCaller.executeOrSendMessage(this.mCaller.obtainMessageO(18, Boolean.valueOf(enable)));
    }
}
