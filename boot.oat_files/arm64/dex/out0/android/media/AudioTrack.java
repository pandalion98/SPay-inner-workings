...
public class AudioTrack {
    ...
    public static class Builder {
        ...
        public AudioTrack build() throws UnsupportedOperationException {
            ...
            if (this.mFormat == null) {
                this.mFormat = new android.media.AudioFormat.Builder().setChannelMask(12).setSampleRate(AudioSystem.getPrimaryOutputSamplingRate()).setEncoding(1).build();
            }
            try {
    ...
    private class NativeRoutingEventHandlerDelegate {
        ...
        NativeRoutingEventHandlerDelegate(AudioTrack track, OnRoutingChangedListener listener, Handler handler) {
            Looper looper;
            if (handler != null) {
                looper = handler.getLooper();
            } else {
    ...
    private final native int native_setup(Object obj, Object obj2, int i, int i2, int i3, int i4, int i5, int i6, int[] iArr);
    ...
    public AudioTrack(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int mode, int sessionId) throws IllegalArgumentException {
        ...
        if (attributes == null) {
            ...
            this.mAttributes = new android.media.AudioAttributes.Builder(attributes).build();
            if (sessionId < 0) {
                throw new IllegalArgumentException("Invalid audio session ID: " + sessionId);
            }
            int[] session = new int[]{sessionId};
            int initResult = native_setup(new WeakReference(this), this.mAttributes, this.mSampleRate, this.mChannelMask, this.mChannelIndexMask, this.mAudioFormat, this.mNativeBufferSizeInBytes, this.mDataLoadMode, session);
            if (initResult != 0) {
    ...
    private void audioParamCheck(int sampleRateInHz, int channelConfig, int channelIndexMask, int audioFormat, int mode) {
        ...
        if (this.mChannelIndexMask != 0) {
            if ((channelIndexMask & Color.YELLOW) != 0) {
    ...
    private boolean isRestricted() {
        if ((this.mAttributes.getAllFlags() & 64) != 0) {
            return false;
        }
        try {
            int mode = this.mAppOps.checkAudioOperation(28, AudioAttributes.usageForLegacyStreamType(this.mStreamType), Process.myUid(), ActivityThread.currentPackageName());
            if (mode != 0) {
