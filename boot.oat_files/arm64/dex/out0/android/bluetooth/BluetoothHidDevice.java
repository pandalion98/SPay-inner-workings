...
public final class BluetoothHidDevice implements BluetoothProfile {
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            if (BluetoothHidDevice.this.mServiceListener != null) {
                BluetoothHidDevice.this.mServiceListener.onServiceConnected(22, BluetoothHidDevice.this);
            }
        }
        ...
        public void onServiceDisconnected(ComponentName className) {
            ...
            BluetoothHidDevice.this.mService = null;
            if (BluetoothHidDevice.this.mServiceListener != null) {
                BluetoothHidDevice.this.mServiceListener.onServiceDisconnected(22);
            }
        }
    };
    ...
    BluetoothHidDevice(Context context, ServiceListener listener) {
        ...
        synchronized (this.mConnection) {
            try {
                ...
                Log.e(TAG, "onBluetoothStateChange: could not bind to HID Dev service: ", e2);
            } catch (SecurityException e3) {
                Log.e(TAG, "onBluetoothStateChange: could not bind to HID Dev service: ", e3);
            }
        }
    }

    boolean doBind() {
        Intent intent = new Intent("com.samsung.bluetooth.IBluetoothHidDevice");
    ...
    public boolean isPreoccupied() {
        Log.v(TAG, "isPreoccupied()");
        boolean result = false;
        if (this.mService != null) {
            try {
                result = this.mService.isPreoccupied();
            } catch (RemoteException e) {
                Log.e(TAG, e.toString());
            }
        } else {
    ...
    public boolean connect_to(BluetoothDevice device) {
        Log.v(TAG, "connect_to(" + device + ")");
        boolean result = false;
        if (this.mService != null) {
            try {
                result = this.mService.connect_to(device);
            } catch (RemoteException e) {
                Log.e(TAG, e.toString());
            }
        } else {
