...
public final class ScanFilter implements Parcelable {
    ...
    public boolean matches(ScanResult scanResult) {
        ...
        if (this.mManufacturerId >= 0) {
            if (scanRecord.isMultiManudata()) {
                Log.d(TAG, "Multi manu data");
                if (!matchesMultiPartialData(this.mManufacturerData, this.mManufacturerDataMask, scanRecord)) {
                    return false;
                }
            } else if (!matchesPartialData(this.mManufacturerData, this.mManufacturerDataMask, scanRecord.getManufacturerSpecificData(this.mManufacturerId))) {
                return false;
            }
        }
        return true;
    ...
    private boolean matchesMultiPartialData(byte[] data, byte[] dataMask, ScanRecord scanRecord) {
        if (scanRecord == null) {
            return false;
        }
        boolean matches = false;
        ArrayList arrayList = new ArrayList();
        ArrayList<byte[]> manuDatalist = scanRecord.getManudataList();
        for (int i = 0; i < manuDatalist.size(); i++) {
            matches = matchesPartialData(data, dataMask, (byte[]) manuDatalist.get(i));
            if (matches) {
                Log.d(TAG, "manu data matches");
                break;
            }
        }
        if (matches) {
