...
public class SSLCertificateSocketFactory extends SSLSocketFactory {
    ...
    public static SSLSocketFactory getDefault(KeyManager[] keyManager, int handshakeTimeoutMillis, SSLSessionCache cache) {
        SSLCertificateSocketFactory ret = new SSLCertificateSocketFactory(handshakeTimeoutMillis, cache, true);
        ret.mKeyManager = keyManager;
        return ret;
    }
    ...
    public static SSLSocketFactory getInsecure(KeyManager[] keyManager, int handshakeTimeoutMillis, SSLSessionCache cache) {
        SSLCertificateSocketFactory ret = new SSLCertificateSocketFactory(handshakeTimeoutMillis, cache, false);
        ret.mKeyManager = keyManager;
        return ret;
    }
    ...
    public static org.apache.http.conn.ssl.SSLSocketFactory getHttpSocketFactory(int handshakeTimeoutMillis, SSLSessionCache cache, KeyManager[] keyManager) {
        SSLCertificateSocketFactory sf = new SSLCertificateSocketFactory(handshakeTimeoutMillis, cache, true);
        sf.mKeyManager = keyManager;
        return new org.apache.http.conn.ssl.SSLSocketFactory(sf);
    }
    ...
    private SSLSocketFactory makeSocketFactory(KeyManager[] keyManagers, TrustManager[] trustManagers) {
        try {
            ...
            sslContext.engineInit(this.mKeyManager, trustManagers, null);
