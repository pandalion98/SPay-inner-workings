...
public class VpnService extends Service {
    ...
    public static Intent prepare(Context context) {
        ...
        } else if (context instanceof GenericVpnContext) {
            GenericVpnContext knoxVPNContext = (GenericVpnContext) context;
            String profile = knoxVPNContext.getVPNProfile();
            boolean isConnecting = knoxVPNContext.getVPNState();
            int profileType = 0;
            try {
                if (getService().prepareEnterpriseVpnExt(knoxVPNContext.getApplicationContext().getPackageName(), profile, isConnecting, knoxVPNContext.isMetaEnabled())) {
                    return null;
                }
