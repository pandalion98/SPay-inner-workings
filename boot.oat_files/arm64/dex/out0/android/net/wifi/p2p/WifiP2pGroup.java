...
public class WifiP2pGroup implements Parcelable {
    ...
    private static final Pattern groupOwnerIpAddrPattern = Pattern.compile(" go_ip_addr=((?:(?:[2][5][0-5]|[2][0-4][0-9]|[0-1][0-9][0-9]|[0-9][0-9]|[0-9])\\.){3}(?:[2][5][0-5]|[2][0-4][0-9]|[0-1][0-9][0-9]|[0-9][0-9]|[0-9]))");
    ...
    private static final Pattern ipAddrPattern = Pattern.compile(" ip_addr=((?:(?:[2][5][0-5]|[2][0-4][0-9]|[0-1][0-9][0-9]|[0-9][0-9]|[0-9])\\.){3}(?:[2][5][0-5]|[2][0-4][0-9]|[0-1][0-9][0-9]|[0-9][0-9]|[0-9]))");
    private List<WifiP2pDevice> mClients = new ArrayList();
    private int mFrequency;
    public String mGroupOwnerStaticIp = ProxyInfo.LOCAL_EXCL_LIST;
    ...
    public String mStaticIp = ProxyInfo.LOCAL_EXCL_LIST;
    ...
    public WifiP2pGroup(String supplicantEvent) throws IllegalArgumentException {
        ...
        if (tokens.length < 3) {
            ...
            if (match.find()) {
                ...
                Matcher matchForIntendedAddr = intendedAddrPattern.matcher(supplicantEvent);
                if (matchForIntendedAddr.find()) {
                    this.mOwner.interfaceAddress = matchForIntendedAddr.group(1);
                }
                Matcher matchForIpAddr = ipAddrPattern.matcher(supplicantEvent);
                if (matchForIpAddr.find()) {
                    this.mStaticIp = matchForIpAddr.group(1);
                }
                Matcher matchForGroupOwnerIpAddr = groupOwnerIpAddrPattern.matcher(supplicantEvent);
                if (matchForGroupOwnerIpAddr.find()) {
                    this.mGroupOwnerStaticIp = matchForGroupOwnerIpAddr.group(1);
                }
            }
    ...
    public boolean removeClient(String address) {
        boolean remove;
        synchronized (this.mClients) {
            remove = this.mClients.remove(new WifiP2pDevice(address));
        }
        return remove;
    }
    ...
    public boolean removeClient(WifiP2pDevice device) {
        boolean remove;
        synchronized (this.mClients) {
            remove = this.mClients.remove(device);
        }
        return remove;
    }
    ...
    public Collection<WifiP2pDevice> getClientList() {
        Collection<WifiP2pDevice> unmodifiableCollection;
        synchronized (this.mClients) {
            unmodifiableCollection = Collections.unmodifiableCollection(this.mClients);
        }
