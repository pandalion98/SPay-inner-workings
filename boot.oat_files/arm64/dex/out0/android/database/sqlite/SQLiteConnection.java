...
public final class SQLiteConnection implements OnCancelListener {
    ...
    private static final String[] bypass_dbs = new String[]{"contacts2.db", "profile.db", "EmailProvider.db", "EmailProviderBody.db"};
    private static final String[] wal_dbs = new String[]{"webview.db", "webviewCache.db", "internal.db", "external.db", "settings.db", "browser.db", "browser2.db", "mmssms.db"};
    private static final String[] wal_dbs_kor = new String[]{"webview.db", "webviewCache.db", "internal.db", "external.db", "settings.db", "browser.db", "browser2.db"};
    ...
    private static native void nativeChangePassword(long j, byte[] bArr);
    ...
    private static native void nativeSetPassword(long j, byte[] bArr);

    public static native int native_backup(String str, String str2);
    ...
    private boolean checkWalDB(String path) {
        for (String s : wal_dbs) {
            if (path.endsWith(s)) {
                return true;
            }
        }
    ...
    private boolean checkBypassDB(String path) {
        for (String s : bypass_dbs) {
            if (path.endsWith(s)) {
                return true;
            }
        }
    ...
    private void open() {
        ...
        if (this.mConnectionPtr != 0) {
            ...
            if (checkWalDB(this.mConfiguration.path)) {
                if (!(this.mConfiguration.isInMemoryDb() || this.mIsReadOnlyConnection)) {
                    setJournalMode("WAL");
                    setSyncMode("NORMAL");
                    setAutoCheckpointInterval(1000);
                    setJournalSizeLimit(8388608);
                }
            } else if (checkBypassDB(this.mConfiguration.path)) {
                setSyncMode("NORMAL");
                setAutoCheckpointInterval(1000);
                setJournalSizeLimit(8388608);
            } else {
    ...
    public void execute(String sql, Object[] bindArgs, CancellationSignal cancellationSignal) {
        ...
        int cookie = this.mRecentOperations.beginOperation(Context.EXECUTE_SERVICE, sql, bindArgs);
        try {
    ...
    public void setPassword(byte[] password) {
        nativeSetPassword(this.mConnectionPtr, password);
    }

    public void changePassword(byte[] newPassword) {
        nativeChangePassword(this.mConnectionPtr, newPassword);
    }
