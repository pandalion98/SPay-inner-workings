...
public class SQLiteCursor extends AbstractWindowedCursor {
    ...
    protected void clearOrCreateWindow(String name) {
        if (this.mWindow == null) {
            int windowSize = 0;
            if (name != null && (name.contains("contacts2.db") || name.contains("EmailProvider.db") || name.contains("EmailProviderBody.db") || name.contains("pen_memo.db") || name.contains("mmssms.db"))) {
                windowSize = 4194304;
            }
            if (windowSize != 0) {
                this.mWindow = new CursorWindow(name, windowSize);
                return;
            } else {
