...
public class GradientDrawable extends Drawable {
    ...
    private static final String LOG_TAG_DCS = "SRIB_DCS_GradientDrawable";
    ...
    static final class GradientState extends ConstantState {
        ...
        public GradientState(GradientState state) {
            ...
            this.mSolidColors = state.mSolidColors;
            if (state.mGradientColors != null) {
                this.mGradientColors = (int[]) state.mGradientColors.clone();
            }
            if (state.mPositions != null) {
                this.mPositions = (float[]) state.mPositions.clone();
            }
            this.mStrokeColors = state.mStrokeColors;
        ...
        public int getChangingConfigurations() {
            int i = 0;
            int changingConfigurations = (this.mSolidColors != null ? this.mSolidColors.getChangingConfigurations() : 0) | (this.mChangingConfigurations | (this.mStrokeColors != null ? this.mStrokeColors.getChangingConfigurations() : 0));
            if (this.mTint != null) {
            ...
            switch (st.mShape) {
                case 0:
                    if (st.mRadiusArray == null) {
                        ...
                        if (DCS_FLAG_APPLY_SHADOW && canvas.getDCSFormat() != -1) {
                            this.mShadowPaint.setStyle(Style.FILL_AND_STROKE);
                            this.mShadowPaint.setColor(-16777216);
                            this.mShadowPaint.setMaskFilter(new BlurMaskFilter(6.0f, Blur.OUTER));
                            canvas.drawRoundRect(this.mRect, rad, rad, this.mShadowPaint);
                        }
                        if (haveStroke) {
        ...
        if (this.mGradientIsDirty) {
            ...
            if (gradientColors != null) {
                ...
                if (st.mGradient == 0) {
                    float x1;
                    float y1;
                    level = st.mUseLevel ? ((float) getLevel()) / SensorManager.LIGHT_OVERCAST : 1.0f;
