...
class RippleForeground extends RippleComponent {
    ...
    private static final TimeInterpolator DECELERATE_INTERPOLATOR = new LogDecelerateInterpolator(SensorManager.LIGHT_SUNRISE, 1.4f, 0.0f);
    ...
    public RippleForeground(RippleDrawable owner, Rect bounds, float startingX, float startingY, boolean isBounded, boolean forceSoftware) {
        ...
        if (isBounded) {
            this.mBoundedRadius = 315.0f + ((float) ((350.0d * Math.random()) * 0.1d));
        } else {
    ...
    protected Animator createSoftwareEnter(boolean fast) {
        int duration = (int) ((1000.0d * Math.sqrt((double) ((this.mTargetRadius / WAVE_TOUCH_DOWN_ACCELERATION) * this.mDensity))) + 0.5d);
    ...
    private int getRadiusExitDuration() {
        return (int) ((1000.0d * Math.sqrt((double) (((this.mTargetRadius - getCurrentRadius()) / 4424.0f) * this.mDensity))) + 0.5d);
    }
    ...
    private int getOpacityExitDuration() {
        return (int) (((1000.0f * this.mOpacity) / WAVE_OPACITY_DECAY_VELOCITY) + 0.5f);
    }
    ...
    protected Animator createSoftwareExit() {
        int radiusDuration = getRadiusExitDuration();
        int originDuration = radiusDuration;
        int opacityDuration = getOpacityExitDuration();
        ObjectAnimator tweenRadius = ObjectAnimator.ofFloat((Object) this, TWEEN_RADIUS, 1.0f);
        tweenRadius.setAutoCancel(true);
