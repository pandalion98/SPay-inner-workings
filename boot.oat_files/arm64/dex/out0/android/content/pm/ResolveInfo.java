...
public class ResolveInfo implements Parcelable {
    ...
    public CharSequence loadLabel(PackageManager pm) {
        if (this.activityInfo != null && this.activityInfo.packageName.startsWith("com.android.stk")) {
            String stkSetupMenu = "gsm.STK_SETUP_MENU";
            String simSlotNum = this.activityInfo.packageName.substring("com.android.stk".length());
            if (simSlotNum.length() != 0 && "2".equals(simSlotNum)) {
                stkSetupMenu = "gsm.STK_SETUP_MENU2";
            }
            if (SystemProperties.get(stkSetupMenu) != null && SystemProperties.get(stkSetupMenu).length() > 0) {
                return SystemProperties.get(stkSetupMenu);
            }
        }
        ...
        if (!(this.resolvePackageName == null || this.labelRes == 0)) {
            label = pm.getCSCPackageItemText(this.resolvePackageName);
            if (label == null) {
        ...
        if (this.labelRes != 0) {
            label = pm.getCSCPackageItemText(ci.icon != 0 ? ci.name : ci.packageName);
            if (label == null) {
    ...
    public Drawable loadIcon(PackageManager pm) {
        ...
        if (!(this.resolvePackageName == null || this.iconResourceId == 0)) {
            dr = pm.getCSCPackageItemIcon(this.resolvePackageName);
            if (dr == null) {
        ...
        ComponentInfo ci = getComponentInfo();
        if (dr == null) {
            dr = pm.getCSCPackageItemIcon(ci.icon != 0 ? ci.name : ci.packageName);
        }
    ...
    public void setPackageWeight(Integer packageWeight) {
        this.mPackageWeight = packageWeight;
    }
