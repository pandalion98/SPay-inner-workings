...
public class CipherManager {
    ...
    /* renamed from: mD */
    private String f35mD;

    /* renamed from: mE */
    private String f36mE;

    /* renamed from: mN */
    private String f37mN;
    ...
    private byte[] extendKey(String key) {
        ...
        int i2 = 0;
        while (i2 < key.length() && i2 < bKey.length) {
            bKey[i2] = bData[i2];
            i2++;
        }
        return bKey;
    ...
    private PublicKey readPubKey() throws IOException {
        try {
            return KeyFactory.getInstance("RSA").generatePublic(new RSAPublicKeySpec(new BigInteger(this.f37mN, 16), new BigInteger(this.f36mE, 16)));
        } catch (Exception e) {
    ...
    private PrivateKey readPrivKey() throws IOException {
        try {
            return KeyFactory.getInstance("RSA").generatePrivate(new RSAPrivateKeySpec(new BigInteger(this.f37mN, 16), new BigInteger(this.f35mD, 16)));
        } catch (Exception e) {
            throw new RuntimeException("Spurious serialisation error", e);
        }
    }
