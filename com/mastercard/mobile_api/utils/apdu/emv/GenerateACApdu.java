...
public class GenerateACApdu extends Apdu {
    ...
    public static final byte P1_AAC = (byte) 0;
    public static final byte P1_CDA = (byte) 16;
    ...
    private void parse(ByteArray byteArray) {
        this.CDOL = byteArray.copyOfRange(5, getLc() + 5);
        ...
        if (getLc() >= 45) {
            this.merchantCategoryCode = byteArray.copyOfRange(48, 50);
        } else {
            this.merchantCategoryCode = ByteArrayFactory.getInstance().fromHexString("0000");
        }
        if (getLc() >= 65) {
            this.merchantNameLocation = byteArray.copyOfRange(50, 70);
        } else {
            this.merchantNameLocation = ByteArrayFactory.getInstance().getByteArray(MERCHANT_NAME_LOC_LEN);
        }
    }
}
