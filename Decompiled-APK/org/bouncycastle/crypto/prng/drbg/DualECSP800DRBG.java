...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    static {
        ...
        nistPoints[0] = new DualECPoints(X509KeyUsage.digitalSignature, fp.createPoint(p256_Px, p256_Py), fp.createPoint(p256_Qx, p256_Qy), 1);
        fp = (Fp) NISTNamedCurves.getByName("P-384").getCurve();
        nistPoints[1] = new DualECPoints(CipherSuite.TLS_RSA_WITH_CAMELLIA_256_CBC_SHA256, fp.createPoint(p384_Px, p384_Py), fp.createPoint(p384_Qx, p384_Qy), 1);
        fp = (Fp) NISTNamedCurves.getByName("P-521").getCurve();
        nistPoints[2] = new DualECPoints(SkeinMac.SKEIN_256, fp.createPoint(p521_Px, p521_Py), fp.createPoint(p521_Qx, p521_Qy), 1);
    }
    ...
    public DualECSP800DRBG(DualECPoints[] dualECPointsArr, Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        } else if (entropySource.entropySize() < i || entropySource.entropySize() > MAX_PERSONALIZATION_STRING) {
            throw new IllegalArgumentException("EntropySource must provide between " + i + " and " + MAX_PERSONALIZATION_STRING + " bits");
        } else {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, SkeinMac.SKEIN_512)) {
            ...
            while (i < length2) {
                ...
                Object toByteArray = getScalarMultipleXCoord(this._Q, scalarMultipleXCoord).toByteArray();
                if (toByteArray.length > this._outlen) {
                    System.arraycopy(toByteArray, toByteArray.length - this._outlen, bArr, i2, this._outlen);
                } else {
                    System.arraycopy(toByteArray, 0, bArr, (this._outlen - toByteArray.length) + i2, toByteArray.length);
                }
                i2 += this._outlen;
                this._reseedCounter++;
                i++;
            ...
            if (i2 < bArr.length) {
                ...
                if (toByteArray2.length > this._outlen) {
                    System.arraycopy(toByteArray2, toByteArray2.length - this._outlen, bArr, i2, length3);
                } else {
                    System.arraycopy(toByteArray2, 0, bArr, i2 + (this._outlen - toByteArray2.length), length3);
                }
                this._reseedCounter++;
            }
