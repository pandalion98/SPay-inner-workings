...
public class MChipCVM_IssuerOptions {
    ...
    public ByteArray getMPAObject() {
        byte by = this.getACK_AlwaysRequiredIfCurrencyProvided() ? MChipCVM_IssuerOptions.setBit((byte)0, 7) : (byte)0;
        if (this.getACK_AlwaysRequiredIfCurrencyNotProvided()) {
            by = MChipCVM_IssuerOptions.setBit(by, 6);
        }
        if (this.getACK_PreEntryAllowed()) {
            by = MChipCVM_IssuerOptions.setBit(by, 5);
        }
        if (this.getPIN_AlwaysRequiredIfCurrencyProvided()) {
            by = MChipCVM_IssuerOptions.setBit(by, 4);
        }
        if (this.getPIN_AlwaysRequiredIfCurrencyNotProvided()) {
            by = MChipCVM_IssuerOptions.setBit(by, 3);
        }
        if (this.getPIN_PreEntryAllowed()) {
            by = MChipCVM_IssuerOptions.setBit(by, 2);
        }
        if (this.getACK_AutomaticallyResetByApplication()) {
            by = MChipCVM_IssuerOptions.setBit(by, 1);
        }
        if (this.getPIN_AutomaticallyResetByApplication()) {
            by = MChipCVM_IssuerOptions.setBit(by, 0);
        }
