...
public class c {
    ...
    public static final List<byte[]> getDerChain(String string) {
        ...
        String[] arrstring = c.splitPem(string);
        for (int i2 = 0; i2 < arrstring.length; ++i2) {
            arrayList.add((Object)Base64.decode((String)c.stripPem(arrstring[i2]), (int)0));
        }
        return arrayList;
    }
    ...
    public static void setUnsigned8ArrayData(Struct.Unsigned8[] arrunsigned8, byte[] arrby) {
        if (arrby == null || arrby.length <= 0) return;
        {
            if (arrby.length > arrunsigned8.length) {
                Log.e((String)"Amex.tzsvc.Utils", (String)"Error: Can't set a buffer more than the size of the destination Byte Array");
                return;
            } else {
    ...
    static String[] splitPem(String string) {
        ...
        for (int i2 = 1; i2 < arrstring.length; ++i2) {
            arrstring2[i2 - 1] = c.stripPem("-----BEGIN CERTIFICATE-----" + arrstring[i2]);
        }
        return arrstring2;
    }
