...
extends m {
    ...
    public void a(ICommonCallback iCommonCallback) {
        ...
        synchronized (t2) {
            if (this.mj) {
                Log.e("TapAndGoDecorator", "No user initiated Payment to stop, Tap N Go is in progress");
    ...
    protected void bg() {
        block3 : {
            block2 : {
                Log.d("TapAndGoDecorator", "Restricted Payment select ppse, TapNGo case");
                if (!o.p(1)) break block2;
                Log.d("TapAndGoDecorator", "Restricted Payment NPAY_READY state");
                c c2 = super.X();
        ...
        Log.d("TapAndGoDecorator", "Restricted Payment No default card avail");
    }

    protected void bh() {
        Log.i("TapAndGoDecorator", "startRestrictedPayment time = " + System.currentTimeMillis());
    ...
    public void clearCard() {
        ...
        synchronized (t2) {
            if (this.mj) {
                Log.e("TapAndGoDecorator", "clearCard(): tapNGo card is in progress, nothing to clear ");
            } else {
                super.clearCard();
            }
            return;
    ...
    public byte[] processApdu(byte[] arrby, Bundle bundle) {
        ...
        synchronized (t2) {
            ...
            synchronized (h3) {
                if (arrby == null) {
                    Log.e("TapAndGoDecorator", "command apdu is null");
                    return com.samsung.android.spayfw.core.b.iR;
                }
                Log.d("TapAndGoDecorator", "processApdu");
                ...
                if (c2 == null || c2.ac() == null || c2.ac().getTokenId() == null) {
                    Log.w("TapAndGoDecorator", "No default card - Samsung pay activity in foreground");
                    return super.processApdu(arrby, bundle);
                }
                Log.w("TapAndGoDecorator", "abort Tap n go - Other Samsung pay activity in foreground");
    ...
    extends IPayCallback.Stub {
        ...
        public void onExtractGiftCardDetail(GiftCardDetail giftCardDetail) {
            Log.w("TapAndGoDecorator", "TapNGoPayCallback onExtractGiftCardDetail");
        }

        @Override
        public void onFail(String string, int n2) {
            Log.w("TapAndGoDecorator", "TapNGoPayCallback onFail " + string + " error: " + n2);
            t.this.mj = false;
        }

        @Override
        public void onFinish(String string, int n2, ApduReasonCode apduReasonCode) {
            Log.d("TapAndGoDecorator", "TapNGoPayCallback onFinish " + string);
            t.this.mj = false;
            if (apduReasonCode == null) {
                Log.e("TapAndGoDecorator", "TapNGoPayCallback onFinish arc null ");
                return;
            }
            Bundle bundle = apduReasonCode.getExtraData();
            if (bundle == null || !bundle.containsKey("status")) {
                Log.e("TapAndGoDecorator", "No TapNGo transaction status ");
                return;
            }
            int n3 = bundle.getInt("status");
            Log.i("TapAndGoDecorator", "PAYFW_KEY_TAP_N_GO_TRANSACTION_ERROR_CODE : " + n3 + ", tokenId: " + string);
            ...
            if (bundle2 != null) {
                Log.d("TapAndGoDecorator", "pdolValues " + bundle2.toString());
        ...
        public void onPay(String string, int n2, int n3) {
            Log.w("TapAndGoDecorator", "TapNGoPayCallback onPay " + string);
        }

        @Override
        public void onPaySwitch(String string, int n2, int n3) {
            Log.w("TapAndGoDecorator", "TapNGoPayCallback onPaySwitch " + string);
        }

        @Override
        public void onRetry(String string, int n2, int n3) {
            Log.w("TapAndGoDecorator", "TapNGoPayCallback onRetry " + string);
        }
    }
    ...
    extends ISelectCardCallback.Stub {
        ...
        @Override
        public void onFail(String string, int n2) {
            Log.e("TapAndGoDecorator", "TapNGoSelectCardCallback onFail " + n2);
        }

        @Override
        public void onSuccess(String string, SelectCardResult selectCardResult) {
            Log.d("TapAndGoDecorator", "TapNGoSelectCardCallback onSuccess");
            t.this.mj = true;
            t.this.bh();
        }
    }
