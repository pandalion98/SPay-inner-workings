...
extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent == null || intent.getAction() == null) {
            Log.e("FraudReceiver", "intent is null");
            return;
        } else {
            Log.d("FraudReceiver", "FraudReciever: " + intent.getAction());
            if ("android.intent.action.PACKAGE_DATA_CLEARED".equals((Object)intent.getAction())) {
                ...
                {
                    Log.d("FraudReceiver", "Fraud: add an [" + string + "] reset record");
                    a a2 = a.x(context);
                    if (a2 != null) {
                        a2.W("app_reset");
                    } else {
                        Log.d("FraudReceiver", "addFDeviceRecord: cannot get data");
                    }
                    context.sendBroadcast(new Intent("com.samsung.android.spay.action.REQUEST_RESET"));
                    Log.i("FraudReceiver", "Sent App Reset Broadcast");
                    return;
                }
            } else {
                if (!"android.intent.action.SIM_STATE_CHANGED".equals((Object)intent.getAction())) return;
                {
                    ...
                    Log.d("FraudReceiver", "FraudReceiver: Sim State = " + string);
                    a a3 = a.x(context);
