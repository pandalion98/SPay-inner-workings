...
class X509SignatureUtil {
    ...
    static String getSignatureName(AlgorithmIdentifier algorithmIdentifier) {
        ...
        if ((provider = Security.getProvider((String)"BC")) != null && (string = provider.getProperty("Alg.Alias.Signature." + algorithmIdentifier.getAlgorithm().getId())) != null) {
            return string;
        }
        Provider[] arrprovider = Security.getProviders();
        while (n != arrprovider.length) {
            String string2 = arrprovider[n].getProperty("Alg.Alias.Signature." + algorithmIdentifier.getAlgorithm().getId());
            if (string2 != null) {
                return string2;
            }
            ++n;
