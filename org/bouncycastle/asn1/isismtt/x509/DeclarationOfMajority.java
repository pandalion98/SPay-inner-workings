...
public class DeclarationOfMajority extends ASN1Object implements ASN1Choice {
    ...
    public DeclarationOfMajority(ASN1GeneralizedTime aSN1GeneralizedTime) {
        this.declaration = new DERTaggedObject(false, dateOfBirth, aSN1GeneralizedTime);
    }

    private DeclarationOfMajority(ASN1TaggedObject aSN1TaggedObject) {
        if (aSN1TaggedObject.getTagNo() > dateOfBirth) {
    ...
    public DeclarationOfMajority(boolean z, String str) {
        if (str.length() > dateOfBirth) {
            throw new IllegalArgumentException("country can only be 2 characters");
        } else if (z) {
            this.declaration = new DERTaggedObject(false, fullAgeAtCountry, new DERSequence(new DERPrintableString(str, true)));
        } else {
            ASN1EncodableVector aSN1EncodableVector = new ASN1EncodableVector();
            aSN1EncodableVector.add(ASN1Boolean.FALSE);
            aSN1EncodableVector.add(new DERPrintableString(str, true));
            this.declaration = new DERTaggedObject(false, fullAgeAtCountry, new DERSequence(aSN1EncodableVector));
        }
    }
    ...
    public ASN1Sequence fullAgeAtCountry() {
        return this.declaration.getTagNo() != fullAgeAtCountry ? null : ASN1Sequence.getInstance(this.declaration, false);
    }

    public ASN1GeneralizedTime getDateOfBirth() {
        return this.declaration.getTagNo() != dateOfBirth ? null : ASN1GeneralizedTime.getInstance(this.declaration, false);
    }
