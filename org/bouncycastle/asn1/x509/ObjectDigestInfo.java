...
public class ObjectDigestInfo extends ASN1Object {
    ...
    private ObjectDigestInfo(ASN1Sequence aSN1Sequence) {
        ...
        this.digestedObjectType = ASN1Enumerated.getInstance(aSN1Sequence.getObjectAt(publicKey));
        if (aSN1Sequence.size() == 4) {
            this.otherObjectTypeID = ASN1ObjectIdentifier.getInstance(aSN1Sequence.getObjectAt(publicKeyCert));
        } else {
            i = publicKey;
        }
        this.digestAlgorithm = AlgorithmIdentifier.getInstance(aSN1Sequence.getObjectAt(i + publicKeyCert));
        this.objectDigest = DERBitString.getInstance(aSN1Sequence.getObjectAt(i + otherObjectDigest));
    }
